...
Checkout.prototype = {
    ...
    _onSectionClick: function(event) {
        var clicked = $(Event.element(event).up().up());
        var clickedClasses = $w(clicked.className);
        for(var i = 0; i < clickedClasses.length; i++) {
            var n = clickedClasses[i].match(/opc-[a-z]*/gi);
            if(n) {
                var section = $(n[0]);
                break;
            }
...
//        var section = $(Event.element(event).up().up());
        if (section && section.hasClassName('allow')) {
            Event.stop(event);
    ...
    gotoSection: function (section, reloadProgressBlock) {
        ...
        $$('.opc-'+section).first().addClassName('allow');
